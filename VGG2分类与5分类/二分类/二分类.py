...
def training(net,train_loader,test_loader,criterion,optimizer,net_path,batches):
    ...
    for epoch in range(10):
        ...
        for i, data in enumerate(train_loader):
            ...
            if (i+1)*batches % 120 == 0:
                print('[{}, {}] loss: {:.3f}' .format(epoch + 1, (i + 1)*batches, running_loss / (120/batches)))
                running_loss = 0.0
                net.eval()
...
def testing_select_model(test_loader, net, criterion, net_path):
    ...
    global pre_rule
    #因为发现错误率比交叉熵小一个量级，于是错误率乘了10之后再加权求和
    new_rule = 0.5 * loss + 0.5 * 10 * (1 - acc)
...
if __name__ == '__main__':
    ...
    valid_test_dataset = datasets.ImageFolder(root='./predicted test_2', transform=my_transform)
    ...
    optimizer = optim.Adam(net.parameters(), lr=0.0003)#优化器
    ...
    #测试部分的代码
    net_path_test = 'new 2 classes_VGG_best.pth'
