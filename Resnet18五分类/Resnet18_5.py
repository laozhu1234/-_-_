...
def tst(net, images, labels, best_acc, criterion):
    with torch.no_grad():
        ...
        if acc >= best_acc:
            best_acc = acc
            torch.save(net, r'./model/Resnet18_5.pth')
    return best_acc
...
def main():
    ...
    train_inputs = np.load("./Resnet18_5/train_img.npy")
    train_inputs = torch.from_numpy(train_inputs)
    train_labels = np.load("./Resnet18_5/train_label.npy")
    train_labels = torch.from_numpy(train_labels)

    test_images = np.load("./Resnet18_5/test_img.npy")
    test_images = torch.from_numpy(test_images)
    test_labels = np.load("./Resnet18_5/test_label.npy")
